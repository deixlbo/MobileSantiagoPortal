...
ALTER TABLE document_requests DROP CONSTRAINT IF EXISTS document_requests_document_type_check;
...
-- Helper functions for RLS policies (optimized to avoid recursion)
...
STABLE
AS $$
  -- Prefer JWT role claim to avoid querying `profiles` (prevents recursion).
...
AS $$
  -- Prefer JWT role claim to avoid querying `profiles` (prevents recursion).
  SELECT (auth.jwt() -> 'user_metadata' ->> 'role') = 'official';
$$;
...
CREATE OR REPLACE FUNCTION is_verified(user_id UUID)
RETURNS BOOLEAN
LANGUAGE SQL
SECURITY DEFINER
STABLE
AS $$
  SELECT verification_status = 'verified' FROM profiles WHERE id = user_id;
$$;

-- Helper function to safely check if a profile ID is an official or admin
...
CREATE POLICY "Residents view own feedback" ON feedback
  FOR SELECT USING (
    auth.uid() = resident_id OR is_official_or_admin(auth.uid())
  );

CREATE POLICY "Residents can create feedback" ON feedback
  FOR INSERT WITH CHECK (
    auth.uid() = resident_id
  );

CREATE POLICY "Residents can update own feedback" ON feedback
  FOR UPDATE USING (
    auth.uid() = resident_id
  );

CREATE POLICY "Residents can delete own feedback" ON feedback
  FOR DELETE USING (
    auth.uid() = resident_id
  );

CREATE POLICY "Officials can manage all feedback" ON feedback
  FOR ALL USING (
    is_official_or_admin(auth.uid())
  );
...
RETURNS TRIGGER AS $$
BEGIN
  -- Avoid querying profiles from inside the trigger to prevent recursion in row-level security checks.
...
INSERT INTO storage.buckets (id, name, owner, public, allowed_mime_types, file_size_limit, created_at, updated_at)
VALUES (
  'resident-uploads',
  'resident-uploads',
  uuid_nil(),
