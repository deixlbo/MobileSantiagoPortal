...
import { createOrdinance, updateOrdinance, deleteOrdinance, publishOrdinance, draftOrdinance } from "@/lib/ordinance-utils"
...
export default function OfficialOrdinancesPage() {
  ...
  const handleCreateOrdinance = async () => {
    ...
    try {
      const createdOrdinance = await createOrdinance({
        number: newOrdinance.number,
        year: newOrdinance.year,
        title: newOrdinance.title,
        fullTitle: newOrdinance.fullTitle,
        status: newOrdinance.status,
        date: newOrdinance.date,
        author: newOrdinance.author,
        whereas: newOrdinance.whereas,
        sections: newOrdinance.sections,
        category: newOrdinance.category,
      })
      setOrdinances([createdOrdinance, ...ordinances])
      setShowCreateDialog(false)
      setFormData({ number: "", year: "2026", title: "", fullTitle: "", whereas: "", sections: "" })
    } catch (error) {
      console.error('Failed to create ordinance:', error)
      alert('Failed to save ordinance')
    }
  }
  ...
  const handleUpdateOrdinance = async () => {
    ...
    try {
      const updatedOrdinance = await updateOrdinance(updated.id, updated)
      setOrdinances(ordinances.map(o => o.id === editingOrdinance.id ? updatedOrdinance : o))
    ...
    >
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
          ...
          <DialogContent className="w-[95vw] sm:w-auto max-h-[95vh] overflow-auto max-w-2xl mx-4 md:mx-auto bg-white">
            <DialogHeader>
      ...
      <Dialog open={!!selectedOrdinance} onOpenChange={() => setSelectedOrdinance(null)}>
        <DialogContent className="w-[95vw] sm:w-auto max-h-[95vh] overflow-auto max-w-lg mx-4 md:mx-auto bg-white">
          <DialogHeader>
