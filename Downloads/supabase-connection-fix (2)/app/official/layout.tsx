...
import { getCurrentUser, getUserRole, getProfile, signOut } from "@/lib/auth"
...
export default function OfficialLayout({
  ...
  const [officialProfile, setOfficialProfile] = useState<any>(null)
      ...
      try {
        ...
        if (currentUser && (role === 'official' || role === 'admin')) {
          ...
          try {
            ...
            console.warn('Profile data skipped due to RLS policy:', profileErr)
          }
        } else {
  ...
  // Skip layout for login pages and register
  if (pathname === "/official/login" || pathname === "/official/login-form" || pathname === "/official/register") {
    return <>{children}</>
  }
      ...
      <div className="mt-auto border-t border-slate-200 p-4 space-y-3 bg-slate-50">
        ...
        >
          <Avatar className="h-12 w-12 ring-2 ring-slate-200">
            <AvatarImage
              src={officialProfile?.avatar_url || '/placeholder-avatar.jpg'}
              alt={officialProfile ? `${officialProfile.first_name} ${officialProfile.last_name}` : 'Official'}
            />
            <AvatarFallback className="bg-emerald-700 text-white font-semibold">
              {(officialProfile ? `${officialProfile.first_name || ''} ${officialProfile.last_name || ''}` : user?.email || 'Official')
                .split(' ')
                .filter(Boolean)
                .map((part) => part[0])
                .join('')
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-slate-900 truncate">
              {officialProfile ? `${officialProfile.first_name || ''} ${officialProfile.last_name || ''}`.trim() : user?.email || 'Official'}
            </p>
            <p className="text-xs text-slate-500 truncate">
              {officialProfile?.position || (officialProfile?.role === 'official' ? 'Barangay Official' : officialProfile?.role) || 'Official'}
            </p>
          </div>
        </Link>
        <button 
