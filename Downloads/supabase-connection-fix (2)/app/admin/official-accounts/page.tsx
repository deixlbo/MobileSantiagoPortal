...
export default function AdminOfficialAccountsPage() {
  ...
  const formatStatus = (status?: string) => {
    ...
    if (normalized === 'deactivated' || normalized === 'inactive') return 'Inactive'
  ...
  const fetchOfficials = async () => {
    try {
      ...
      if (!response.ok || result.error) {
        throw new Error(result.error || 'Failed to load official users')
      }
      ...
      setOfficials(
        (result.data || []).map((profile: any) => ({
          ...
          status: formatStatus(profile.verification_status),
        }))
      )
    } catch (error) {
      const message = getErrorMessage(error, 'Failed to load official users')
      console.error('Failed to load official users:', message)
      toast.error(message)
    }
  }
        ...
        <Card className="space-y-4">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-4">
              ...
              <div>
                <Label htmlFor="official-role">Position</Label>
                <select
                  id="official-role"
                  value={role}
