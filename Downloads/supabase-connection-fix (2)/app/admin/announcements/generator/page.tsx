...
export default function AnnouncementGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    const generatedContent = `Important announcement: ${topic}. Please stay informed and follow the instructions provided by barangay officials regarding this matter.`
    ...
    try {
      ...
      if (response.ok && data.success && data.announcement) {
        ...
        toast.error(String(getErrorMessage(data.error || data.message, 'Failed to generate announcement')));
      }
    } catch (error) {
  ...
  const handlePublish = async () => {
    if (!announcement?.id) {
      toast.error('No announcement available to publish');
      return;
    }
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        toast.error(String(getErrorMessage(data.error || data.message, 'Failed to publish announcement')));
      }
    } catch (error) {
      console.error('Error:', error);
