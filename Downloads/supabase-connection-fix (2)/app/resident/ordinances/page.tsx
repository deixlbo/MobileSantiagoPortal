...
export default function OrdinancesPage() {
  ...
  const fetchOrdinances = async () => {
    try {
      const response = await fetch('/api/ordinances?published=true')
      if (!response.ok) throw new Error('Failed to load ordinances')
  ...
  const filteredOrdinances = ordinances.filter(ord => 
    String(ord.title || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
    String(ord.number || '').includes(searchTerm)
  )
            ...
            <ScrollArea className="max-h-[65vh] sm:max-h-[70vh]">
              <div className="rounded-lg border border-gray-200 bg-white p-3 sm:p-6 md:p-8 text-gray-900">
                ...
                <div className="mb-6">
                  ...
                  {selectedOrdinance.whereas?.map((clause: string, i: number) => (
                ...
                {/* Sections */}
                {selectedOrdinance.sections?.map((section: any, i: number) => (
