...
async function setupDatabase() {
  ...
  try {
    ...
    const bucketNames = bucketsResult.rows.map((row) => row.name);
    console.log('\n💾 Storage:');
    console.log(`  • resident-uploads bucket: ${bucketNames.includes('resident-uploads') ? 'configured' : 'missing'}`);
    console.log(`  • assets bucket: ${bucketNames.includes('assets') ? 'configured' : 'missing'}`);
