...
const documentTypes = [
  ...
  { 
    id: "barangay_business_clearance", 
...
export default function DocumentsPage() {
  ...
  const handleDownloadQRCode = () => {
    ...
    try {
      ...
      a.href = url
      a.download = `${selectedRequest?.id ?? 'document'}-qr.png`
      a.click()
    } catch (e) {
  ...
  const handleSubmitRequest = async () => {
    ...
    if (requirements.some(r => r.status === 'empty')) {
    ...
    try {
      const user = await getCurrentUser()
      if (!user?.id) {
        toast.error('Unable to identify current resident. Please log in again.')
        return
      }

      const formData = new FormData()
      formData.append('residentId', user.id)
      formData.append('documentType', selectedType)
      formData.append('purpose', purpose)
      formData.append('barangayCaptan', 'Rolando C. Borja')

      requirements.forEach((req) => {
        if (req.file) {
          formData.append(`file_${req.name}`, req.file)
        }
      })
      ...
      if (!response.ok || result.error) {
        toast.error(result.error || 'Failed to submit document request.')
        return
      }
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button 
              ...
              disabled={!selectedType || requirements.some(r => r.status === "empty") || isSubmitting}
  ...
  <Dialog open={Boolean(selectedRequest)} onOpenChange={(open) => { if (!open) setSelectedRequest(null) }}>
    <DialogContent className="max-w-sm sm:max-w-md">
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          {selectedRequest.status !== 'rejected' ? (
            <>
              ...
              <div className="w-full max-w-xs">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg border border-border/50 bg-slate-50 p-2 text-xs">
                    <p className="text-muted-foreground text-xs">Request Number</p>
                    <p className="font-medium truncate">{selectedRequest.id}</p>
                  </div>
                  <div className="rounded-lg border border-border/50 bg-slate-50 p-2 text-xs">
                    <p className="text-muted-foreground text-xs">Document Type</p>
                    <p className="font-medium truncate">{selectedRequest.type}</p>
                  </div>
                  <div className="rounded-lg border border-border/50 bg-slate-50 p-2 text-xs">
            ...
            <div className="w-full max-w-xs space-y-3">
              <div className="p-3 bg-red-50 rounded-lg w-full text-left">
                <p className="text-sm text-destructive flex items-start gap-2">
                  <AlertCircle className="h-4 w-4 flex-shrink-0 mt-0.5" />
                  <span>Reason: {selectedRequest.remarks ?? 'No reason provided'}</span>
                </p>
              </div>
              ...
              <div className="w-full">
                <Button onClick={() => { setShowResubmit(selectedRequest); setSelectedRequest(null); }} className="w-full">
                  Resubmit
                </Button>
              </div>
            </div>
      ...
      <DialogFooter className="flex-col sm:flex-row gap-2">
        <Button variant="outline" onClick={handleDownloadQRCode} disabled={selectedRequest?.status === 'rejected'} className="w-full sm:w-auto">
