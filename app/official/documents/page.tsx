...
const defaultDocumentTypes: DocumentType[] = [
  { id: "1", name: "Barangay Clearance", requirements: "Valid ID, Proof of Residency", fee: "50", isActive: true },
  { id: "2", name: "Certificate of Residency", requirements: "Proof of Address, Valid ID", fee: "30", isActive: true },
  { id: "3", name: "Business Clearance", requirements: "Business Registration, Valid ID", fee: "200", isActive: true },
  { id: "4", name: "Certificate of Indigency", requirements: "Proof of Residency, Income Statement", fee: "Free", isActive: true },
]

const sampleOfficialRequests: DocumentRequest[] = [
  {
    id: "1",
    request_number: "DOC-2024-001",
    type: "Barangay Clearance",
    purpose: "Employment",
    requester_id: "resident1",
    ...
    document_path: [
      { requirement: "Valid ID", name: "ID_Front.pdf", url: "#" },
      { requirement: "Proof of Residency", name: "Lease_Agreement.pdf", url: "#" }
    ],
  ...
  {
    id: "2",
    request_number: "DOC-2024-002",
    type: "Certificate of Residency",
    purpose: "School Enrollment",
    requester_id: "resident2",
    ...
    missing_documents: null,
    document_path: [
      { requirement: "Proof of Address", name: "Address_Proof.pdf", url: "#" }
    ],
  ...
  {
    id: "3",
    request_number: "DOC-2024-003",
    type: "Barangay Clearance",
    purpose: "Travel",
    requester_id: "resident3",
    requester_name: "Pedro Reyes",
    requester_email: "pedro@example.com",
    purok: "Purok 3",
    status: "released",
    fee: "50",
    pickup_time: new Date(Date.now() + 1 * 24 * 60 * 60 * 1000).toISOString(),
...
export default function OfficialDocumentsPage() {
  ...
  const handleAddType = () => {
    if (!formData.name || !formData.requirements || !formData.fee) {
  ...
  const handleEditType = () => {
    if (!editingType || !formData.name || !formData.requirements || !formData.fee) {
      toast.error("Please fill in all fields")
      return
    }
    ...
    setDocumentTypes(documentTypes.map(type => 
      type.id === editingType.id 
        ? { ...type, name: formData.name, requirements: formData.requirements, fee: formData.fee }
        : type
    ))
    setEditingType(null)
    setFormData({ name: "", requirements: "", fee: "" })
  ...
  const handleDeactivateType = (typeId: string) => {
    setDocumentTypes(documentTypes.map(type =>
      type.id === typeId ? { ...type, isActive: !type.isActive } : type
    ))
    toast.success(`Document type ${documentTypes.find(t => t.id === typeId)?.isActive ? 'deactivated' : 'activated'}`)
  }
