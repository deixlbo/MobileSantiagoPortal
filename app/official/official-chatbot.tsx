...
function getOfficialResponse(question: string): string {
  ...
  if ((lowerQuestion.includes("ilang") || lowerQuestion.includes("how many")) && lowerQuestion.includes("process")) {
    return "Ngayong araw, na-process na ang 15 document requests: 8 Barangay Clearance (approved), 5 Certificate of Residency (approved), at 2 Certificate of Indigency (approved). May 3 declined requests dahil sa incomplete requirements."
  }
  
  if (lowerQuestion.includes("today") || (lowerQuestion.includes("ngayon") && lowerQuestion.includes("transactions"))) {
    return "Narito ang summary ng transactions ngayong araw: 15 approved requests, 3 declined requests, 5 new blotter reports filed, at 2 blotter cases resolved. Total collection: Php 1,250.00 mula sa processing fees."
  }
