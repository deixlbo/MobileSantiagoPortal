...
export default function AdminForgotPasswordPage() {
  ...
  useEffect(() => {
    const { secondsRemaining } = checkRateLimit('admin-forgot-password', email || 'general')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { allowed, secondsRemaining } = checkRateLimit('admin-forgot-password', email)
    if (!allowed) {
    ...
    try {
      ...
      if (!response.ok) {
        const message = result.error || 'Unable to send reset email. Please try again.'
        
        if (message.includes('Too many') || message.includes('rate')) {
          setRateLimit('admin-forgot-password', email, 3600)
          setCooldown(3600)
      ...
      setRateLimit('admin-forgot-password', email, 300)
      setCooldown(300)
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-white text-slate-950">
      <div className="pointer-events-none absolute left-0 top-0 h-[30rem] w-[30rem] -translate-x-1/3 -translate-y-1/3 rounded-full bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.28),transparent_55%)] blur-3xl" />
      <div className="pointer-events-none absolute right-0 top-24 h-[28rem] w-[28rem] rounded-full bg-[radial-gradient(circle_at_top_right,_rgba(37,99,235,0.20),transparent_55%)] blur-3xl" />
      <div className="pointer-events-none absolute left-0 bottom-0 h-[24rem] w-[24rem] rounded-full bg-[radial-gradient(circle_at_bottom_left,_rgba(66,133,244,0.18),transparent_55%)] blur-3xl" />
      ...
      <div className="relative mx-auto flex min-h-screen max-w-[1200px] flex-col justify-center px-8 py-10 sm:px-10 lg:px-16 xl:px-24">
        <div className="mx-auto w-full max-w-3xl rounded-[38px] border border-slate-200/70 bg-white/95 p-10 shadow-[0_40px_120px_-60px_rgba(15,23,42,0.18)] backdrop-blur-xl">
          <div className="mb-10 flex items-center gap-4">
            <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-3xl bg-blue-50 shadow-sm shadow-blue-100">
              <Image
            ...
            <div>
              <p className="text-sm uppercase tracking-[0.35em] text-blue-700">Admin Password Recovery</p>
          ...
          <p className="mb-8 max-w-2xl text-sm leading-6 text-slate-600">
            Enter the email address for your admin account and we&apos;ll send a secure password reset link. Use the link to set a new password and sign in again.
          </p>
