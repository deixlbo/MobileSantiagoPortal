...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!supabase) {
      console.log('[v0] Analytics insights returning mock data - database not configured')
      return NextResponse.json({
        success: true,
        message: 'Using sample insights data',
        fallback: true,
        insights: [
          { title: 'High Document Processing Rate', description: 'Barangay processed 340 documents this month', metric: 340, unit: 'documents', trend: 'up' },
          { title: 'Strong Verification Rate', description: '94% of documents are verified within SLA', metric: 94, unit: '%', trend: 'up' },
          { title: 'Growing User Base', description: '1,250 active users this month', metric: 1250, unit: 'users', trend: 'up' }
        ]
      })
    }
