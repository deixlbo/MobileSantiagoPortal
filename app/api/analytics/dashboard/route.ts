...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!supabase) {
      console.log('[v0] Analytics returning mock data - database not configured')
      return NextResponse.json({
        success: true,
        message: 'Using sample analytics data',
        fallback: true,
        metrics: {
          activeUsers: { total: 1250, change: 12 },
          documentsProcessed: { total: 340, change: 8 },
          certificatesIssued: { total: 156, change: 5 },
          totalNotifications: 2840,
          verificationSuccess: { percentage: 94, trend: 2 },
          engagement: { score: 78, trend: 3 }
        },
        trends: {
          activeUsers: 12,
